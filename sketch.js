...
function handleOrientation(event) {
  ...
  // Apply gradient and update position on mobile devices if movement exceeds threshold
  if (isMobileDevice() && previousPosition) {
    ...
    if (distance > MOVE_THRESHOLD) {
      ...
      // **꽃 트레일 추가** (여기서 꽃을 추가합니다)
      drawFlower(shibaX, shibaY, currentAzimuth);
    }
  }
}
...
function applyGradientEffect(distance, azimuth) {
  ...
  // Draw multiple shapes to create a gradient effect
  for (let i = 0; i < 10; i++) {

...
function modelLoaded() {
  console.log("Shiba 모델 로딩 성공");
}

function modelLoadError(err) {
  console.error("Shiba 모델 로딩 실패:", err);
}

function textureLoaded() {
  console.log("Shiba 텍스처 로딩 성공");
}

function textureLoadError(err) {
  console.error("Shiba 텍스처 로딩 실패:", err);
}
...
function applyGradientEffect(distance, azimuth) {
  ...
  // 방위각을 이용해 그라데이션 위치 계산 (Shiba Inu의 현재 위치 기준)

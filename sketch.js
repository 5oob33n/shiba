...
function handleOrientation(event) {
  ...
  // Rotate Shiba Inu's head only, do not move position
  // No movement logic here to prevent unintended movement when only rotating
}
...
function updatePosition(position) {
  ...
  if (previousPosition) {
    ...
    if (distance > MOVE_THRESHOLD) {
      ...
      // Apply gradient effect (existing green and blue gradients)
      applyGradientEffect(distance, currentAzimuth);

      // Draw flower trail
      drawFlowerTrail(shibaX, shibaY, currentAzimuth);
    }
  }
...
// Function to draw a flower at (x, y) with a given size
function drawFlower(x, y, azimuth) {
  ...
  push();
  translate(x, y);
  rotate(angleOffset); // Rotate flower based on azimuth

  fill(255, 192, 203, 150); // Hot Pink color for flowers
  noStroke();
  ...
  for (let i = 0; i < petals; i++) {
    ...
    push();
    rotate(angle);
    ellipse(0, -radius, petalWidth, petalLength);
    pop();
  }

  // Draw the center of the flower
  fill(255, 215, 0, 150); // Gold color
  ellipse(0, 0, radius, radius);
  pop();
}

// Function to draw flower trail when moving
function drawFlowerTrail(x, y, azimuth) {
  // Randomly decide to draw a flower to add variety
  if (random(1) < 0.7) { // 70% chance to draw a flower
    let size = random(8, 12); // Random size for variety
    drawFlower(width / 2 + x, height / 2 + y, azimuth);
  }
}

function applyGradientEffect(distance, azimuth) {
  // Select a random color from green and blue gradients
  let gradientColors = [
  ...
  // Draw gradient shapes (ellipse and star)
  for (let i = 0; i < 10; i++) {
...
// Function to draw a star at (x, y) with inner and outer radii and number of points
...
function drawNaturalElements() {
  if (currentRegion === 'forest') {
    ...
    for (let i = 0; i < 5; i++) {
      paintLayer.fill(34, 139, 34, 150); // Forest Green
    ...
    for (let i = 0; i < 5; i++) {
      paintLayer.fill(30, 144, 255, 150); // Dodger Blue
